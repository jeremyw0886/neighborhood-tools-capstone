...
#observer;
#counters;#abortController=new AbortController();
...
for(const counter of this.#counters)CounterAnimation.#animateCounter(counter);}},{threshold:0.5});
const leftCol=list.parentElement;
if(leftCol?.classList.contains('animate-in')){
leftCol.addEventListener('animationend',
()=>this.#observer.observe(list),{once:true,signal:this.#abortController.signal});}else{
this.#observer.observe(list);}}
static init(){
if(CounterAnimation.#instance)return CounterAnimation.#instance;
if(matchMedia('(prefers-reduced-motion: reduce)').matches)return null;
const list=document.querySelector('.home-page > header > section > div > div:first-child > ul');
if(!list)return null;
...
static#instance=null;
static#LINES=['Borrow tools from your neighbors. Lend yours when you\u2019re not using them.','Save money. Reduce waste. Build trust.','Your neighborhood\u2019s shared toolbox is just a click away.',];
static#INTERVAL=6000;
static#FADE=300;
#subtitle;#current=0;#cycleTimer=null;#fadeTimer=null;#abortController=new AbortController();
...
if(matchMedia('(prefers-reduced-motion: reduce)').matches)return null;
const subtitle=document.querySelector('.home-page > header > section > div > div:first-child > p');
...
this.#abortController.abort();
SubtitleRotator.#instance=null;}#cycle(){
...
this.#cycle();},SubtitleRotator.#FADE);},SubtitleRotator.#INTERVAL);}#handleVisibility=()=>{
if(document.hidden){
clearTimeout(this.#cycleTimer);
clearTimeout(this.#fadeTimer);
this.#cycleTimer=null;}else{
this.#subtitle.classList.remove('fading');
...
#dots;
#observer;
#mq;#abortController=new AbortController();
constructor(section){
...
this.#cards[i].scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});},{signal});
...
this.#dots[i].setAttribute('aria-current',isActive?'true':'false');}}},{root:this.#grid,threshold:0.5});
...
this.#abortController.abort();
this.#nav.remove();
NeighborCarousel.#instance=null;}#activate(){
this.#grid.after(this.#nav);
for(const card of this.#cards)this.#observer.observe(card);
this.#dots[0].classList.add('active');
this.#dots[0].setAttribute('aria-current','true');}#deactivate(){
this.#nav.remove();
for(const card of this.#cards)this.#observer.unobserve(card);
for(const dot of this.#dots){
dot.classList.remove('active');
dot.removeAttribute('aria-current');}}#handleViewport=(e)=>{
if(e.matches)this.#activate();
else this.#deactivate();};}
class ToolPreview{
static#instance=null;
#container;
#tooltip;#activeCard=null;#hideTimeout=null;#abortController=new AbortController();
...
ToolPreview.#instance=null;}#bind(){
const{signal}=this.#abortController;
...
static#cardFrom(el){
return el?.closest?.('article');}#show(card){
...
this.#position(card);}#hide(){
this.#hideTimeout=setTimeout(()=>{
this.#tooltip.hidden=true;
this.#activeCard=null;
if(window.NT)NT.style.removeRule('tool-preview-pos');},80);}#position(card){
...
if(window.NT){
NT.style.setRule('tool-preview-pos','.tool-preview',
`left:${centerX}px;top:${top}px;transform:${above ? 'translate(-50%,-100%)' : 'translateX(-50%)'}`);}}#handlePointerOver=(e)=>{
if(e.pointerType==='touch')return;
const card=ToolPreview.#cardFrom(e.target);
if(card&&card!==this.#activeCard)this.#show(card);};#handlePointerOut=(e)=>{
if(e.pointerType==='touch')return;
const card=ToolPreview.#cardFrom(e.target);
if(!card||card!==this.#activeCard)return;
const related=ToolPreview.#cardFrom(e.relatedTarget);
if(related!==card)this.#hide();};#handleFocusIn=(e)=>{
const card=ToolPreview.#cardFrom(e.target);
if(card)this.#show(card);};#handleFocusOut=(e)=>{
const card=ToolPreview.#cardFrom(e.target);
if(card&&card===this.#activeCard)this.#hide();};#handleEscape=(e)=>{
if(e.key==='Escape'&&this.#activeCard){
this.#tooltip.hidden=true;
this.#activeCard=null;
if(window.NT)NT.style.removeRule('tool-preview-pos');}};#handleScroll=()=>{
...
#memberList;
#links;
#lastLink;#abortController=new AbortController();
#fetchController=null;#ariaLiveTimer=null;
...
this.#abortController.abort();
LocationToggle.#instance=null;}#handleClick=async(e)=>{
e.preventDefault();
...
this.#memberList.dispatchEvent(new CustomEvent('member-list:refresh'));}}catch(err){
if(err.name!=='AbortError'){
const status=document.createElement('p');
status.setAttribute('role','status');
status.textContent='Refreshing\u2026';
this.#memberList.replaceChildren(status);
window.location.href=link.href;}}finally{
...
#nextBtn;
#desktop;
#reducedMotion;#rafPending=false;#abortController=new AbortController();
...
destroy(){
this.#abortController.abort();
MemberCarousel.#instance=null;}#bind(){
const{signal}=this.#abortController;
...
this.#desktop.addEventListener('change',this.#handleViewport,{signal});}#updateArrowState(){
this.#prevBtn.disabled=this.#memberList.scrollLeft<=1;
this.#nextBtn.disabled=
this.#memberList.scrollLeft+this.#memberList.clientWidth>=this.#memberList.scrollWidth-1;}#resetCarousel(){
requestAnimationFrame(()=>{
this.#memberList.scrollLeft=0;
this.#updateArrowState();});}#scrollByCards(direction){
...
behavior:this.#reducedMotion.matches?'auto':'smooth',});}#activate(){
this.#prevBtn.hidden=false;
this.#nextBtn.hidden=false;
this.#memberList.dataset.arrows='';
this.#resetCarousel();}#deactivate(){
this.#prevBtn.hidden=true;
this.#nextBtn.hidden=true;
delete this.#memberList.dataset.arrows;
this.#resetCarousel();}#handlePrev=()=>this.#scrollByCards(-1);#handleNext=()=>this.#scrollByCards(1);#handleRefresh=()=>{
requestAnimationFrame(()=>this.#resetCarousel());};#handleScroll=()=>{
if(this.#rafPending)return;
this.#rafPending=true;
requestAnimationFrame(()=>{
this.#updateArrowState();
this.#rafPending=false;});};#handleViewport=(e)=>{
if(e.matches)this.#activate();
else this.#deactivate();};}
...
destroy(){
this.#abortController.abort();
PopularCarousel.#instance=null;}#bind(){
const{signal}=this.#abortController;
this.#prevBtn.addEventListener('click',this.#handlePrev,{signal});
this.#nextBtn.addEventListener('click',this.#handleNext,{signal});
...
this.#nextBtn.disabled=
this.#list.scrollLeft+this.#list.clientWidth>=this.#list.scrollWidth-1;}#scrollByCards(direction){
...
this.#updateArrowState();});}#deactivate(){
this.#prevBtn.hidden=true;
this.#nextBtn.hidden=true;
delete this.#list.dataset.arrows;
requestAnimationFrame(()=>{
this.#list.scrollLeft=0;
this.#updateArrowState();});}#handlePrev=()=>this.#scrollByCards(-1);#handleNext=()=>this.#scrollByCards(1);#handleScroll=()=>{
if(this.#rafPending)return;
this.#rafPending=true;
requestAnimationFrame(()=>{
this.#updateArrowState();
