...
`enshrined/svg-sanitize` is declared in `composer.json` under `require` and is installed by `composer install` (or `composer update enshrined/svg-sanitize` if it was added without an accompanying lockfile bump). It is **not** optional — without it the admin SVG upload paths under `/admin/images` silently reject every submission. If the dependency is missing at runtime, `SvgSanitizer::sanitize()` returns `null`, the upload controller deletes the partial file and shows a generic rejection flash, and `error_log` records `SvgSanitizer: enshrined/svg-sanitize is not installed; rejecting upload. Run \`composer install\`.` — fail-closed posture.
