...
observer.disconnect();
for(const counter of counters)animateCounter(counter);}},{threshold:0.5});
function startObserving(){
observer.observe(list);}
const leftCol=list.parentElement;
if(leftCol?.classList.contains('animate-in')){
leftCol.addEventListener('animationend',startObserving,{once:true});}else{
startObserving();}})();
(function(){
...
const reducedMotion=window.matchMedia('(prefers-reduced-motion: reduce)');
if(reducedMotion.matches)return;
const lines=['Borrow tools from your neighbors. Lend yours when you\u2019re not using them.','Save money. Reduce waste. Build trust.','Your neighborhood\u2019s shared toolbox is just a click away.',];
let current=0;
const INTERVAL=6000;
const FADE=300;
let timer=null;
...
dots[i].setAttribute('aria-current',isActive?'true':'false');}}},{root:grid,threshold:0.5});
function activate(){
grid.after(nav);
...
if(activeCard)position(activeCard);},{passive:true});})();
(function(){
...
memberList.dispatchEvent(new CustomEvent('member-list:refresh'));}}catch(err){
