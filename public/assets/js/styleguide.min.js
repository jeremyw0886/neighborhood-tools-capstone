...
return(TocHighlighter.#instance=new TocHighlighter(sections,links));}#onIntersect=(entries)=>{
...
return(SwatchCopier.#instance=new SwatchCopier(root,liveRegion));}#onClick=(event)=>{
const swatch=event.target.closest('.swatch-grid > li');
if(!swatch)return;
this.#copy(swatch);};#onKeydown=(event)=>{
...
this.#resetTimer=null;},SwatchCopier.#RESET_MS);}
destroy(){
...
TocHighlighter.init();
SwatchCopier.init();},{once:true});}else{
TocHighlighter.init();
SwatchCopier.init();}