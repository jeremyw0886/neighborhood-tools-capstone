...
return this.#menu.classList.contains('open');}#open(){
...
this.#close(false);}};#handleViewportChange=(e)=>{
if(e.matches){
...
document.addEventListener('click',this.#handleOutsideClick,{signal});}#getItems(){
...
#bellLink;#interval;#consecutiveErrors=0;#timerId=null;#abortController=new AbortController();
...
this.#abortController.abort();
BadgePoller.#instance=null;}#updateBadge(count){
...
this.#bellLink.setAttribute('aria-label',`Notifications (${count} unread)`);}else{
badge?.remove();
...
mobileBell.append(count>0?` Notifications (${count})`:' Notifications');}}#poll=async()=>{
try{
...
document.addEventListener('click',this.#handleOutsideClick,{signal});
this.#dropdown.addEventListener('keydown',this.#handleKeydown,{signal});}#isOpen(){
...
e.preventDefault();
this.#isOpen()?this.#close():this.#open();};#handleOutsideClick=(e)=>{
