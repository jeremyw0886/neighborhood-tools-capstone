# [NeighborhoodTools.org](https://neighborhoodtools.org) Complete Documentation
...
11. [Stored Procedures](#stored-procedures)
    ...
    - [Borrow Expiration Procedures](#116-borrow-expiration-procedures)
    - [Maintenance Procedures](#117-maintenance-procedures)
    - [Deposit Procedures](#118-deposit-procedures)
    - [Search & Query Procedures](#119-search--query-procedures)
    - [Bookmark Procedures](#1110-bookmark-procedures)
...
13. [Scheduled Events](#scheduled-events)
    ...
    - [Borrow Expiration Events](#133-borrow-expiration-events)
    - [Maintenance Events](#134-maintenance-events)
...
NeighborhoodTools.org is a community-based tool lending platform that enables
...
> - **Boolean storage:** All boolean-style flags are physically stored as `TINYINT UNSIGNED` (`1` = true, `0` = false). Where this documentation labels a column type as `boolean`, the on-disk type is `TINYINT UNSIGNED NOT NULL` with default `0` (or `1` where stated). MySQL has no native BOOLEAN type — `BOOL` / `BOOLEAN` are synonyms for `TINYINT(1)`.
...
| Group                              | Tables                                                                                                                                                                                                                                                     |
|------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Accounts**                       | `role_rol`, `account_status_ast`, `contact_preference_cpr`, `state_sta`, `neighborhood_nbh`, `neighborhood_meta_nbm`, `zip_code_zpc`, `account_acc`, `account_meta_acm`, `account_image_aim`, `account_bio_abi`, `password_reset_pwr`, `avatar_vector_avv`  |
| **Tools**                          | `category_cat`, `tool_condition_tcd`, `fuel_type_ftp`, `tool_tol`, `tool_image_tim`, `tool_meta_tlm`                                                                                                                                                       |
...
| **User Interactions**              | `notification_ntf`, `notification_type_ntt`, `notification_preference_ntp`, `search_log_slg`, `event_evt`, `event_meta_evm`, `event_attendee_eya`                                                                                                          |
| **Shared Assets**                  | `vector_image_vec`                                                                                                                                                                                                                                         |
| **Junction Tables**                | `tool_category_tolcat`, `tool_bookmark_acctol`, `neighborhood_zip_nbhzpc`                                                                                                                                                                                  |
| **Future Expansion**               | `phpbb_integration_php`, `audit_log_aud`, `audit_log_detail_ald`                                                                                                                                                                                           |
...
| **Payments & Deposits**            | `deposit_status_dps`, `payment_provider_ppv`, `payment_transaction_type_ptt`, `security_deposit_sdp`, `payment_transaction_ptx`, `payment_transaction_meta_ptm`                                                                                            |
...
Fuel/power source for tools that require it.

| Column          | Type        | Constraints        | Notes                                                                                     |
|-----------------|-------------|--------------------|-------------------------------------------------------------------------------------------|
| `id_ftp`        | int         | PK, auto-increment | -                                                                                         |
| `fuel_name_ftp` | varchar(30) | unique, not null   | Values: gasoline, diesel, propane, two-stroke mix, electric/battery, kerosene, natural gas |
...
| `type_name_ntt` | varchar(30) | unique, not null   | Values: request, approval, due, return, rating, denial, role_change  |
...
| `is_active_ppv`     | tinyint unsigned | not null, default: 1    | Whether provider is available   |

---

### payment_transaction_type_ptt

Payment transaction type lookup. Replaces the former free-form
`transaction_type_ptx VARCHAR` column on `payment_transaction_ptx` with a
normalized FK reference.

| Column          | Type             | Constraints        | Notes                                                                                |
|-----------------|------------------|--------------------|--------------------------------------------------------------------------------------|
| `id_ptt`        | tinyint unsigned | PK, auto-increment | -                                                                                    |
| `type_name_ptt` | varchar(30)      | unique, not null   | Values: deposit_hold, rental_fee, deposit_release, deposit_forfeit, refund           |

> **Note:**
>
> - Lookup is referenced by `payment_transaction_ptx.id_ptt_ptx` (FK).
> - Protected by lookup-table triggers — INSERTs allowed, UPDATEs/DELETEs rejected once seeded.
...
| `is_purged_acc`       | tinyint unsigned | not null, default: 0 | Set to 1 when soft-deleted account has been purged of PII (GDPR/retention)  |
...
| `id_avv_acc`          | int              | -                    | FK to avatar_vector_avv; NULL = no vector chosen                            |
...
- `idx_avatar_vector_acc` on `id_avv_acc`
...
| `width_aim`        | smallint unsigned | -                     | Intrinsic width in pixels after EXIF auto-orient (square crop)              |
| `alt_text_aim`     | varchar(255)      | -                     | -                                                                           |
| `is_primary_aim`   | tinyint unsigned  | not null, default: 0  | -                                                                           |
| `focal_x_aim`      | tinyint unsigned  | not null, default: 50 | Horizontal focal point 0-100 for CSS `object-position`                      |
| `focal_y_aim`      | tinyint unsigned  | not null, default: 50 | Vertical focal point 0-100 for CSS `object-position`                        |
...
SVG avatar vectors selectable by users for their profile picture.

| Column                 | Type         | Constraints        | Notes                                 |
|------------------------|--------------|--------------------|---------------------------------------|
| `id_avv`               | int          | PK, auto-increment | -                                     |
| `file_name_avv`        | varchar(255) | not null           | SVG filename                          |
| `description_text_avv` | varchar(255) | -                  | -                                     |
| `is_active_avv`        | boolean      | not null, default: true | Whether avatar is selectable     |
| `id_acc_avv`           | int          | not null           | FK to account_acc (uploader)          |
| `uploaded_at_avv`      | timestamp    | default: now()     | -                                     |

**Indexes:**

- `idx_active_avv` on `is_active_avv`
- `idx_uploader_avv` on `id_acc_avv`

> **Note:**
>
> - FK to account_acc on delete RESTRICT (cannot delete account that uploaded vectors).
> - account_acc.id_avv_acc references this table via a deferred ALTER TABLE FK (circular dependency).
...
| `is_available_tol`                | tinyint unsigned | not null, default: 1  | Owner listing toggle - see Note for true availability logic            |
| `is_deleted_tol`                  | tinyint unsigned | not null, default: 0  | Soft-delete flag — excluded from all views                             |
| `deleted_at_tol`                  | timestamp        | -                     | When soft-deleted; drives 90-day deferred image cleanup                |
| `is_deposit_required_tol`         | tinyint unsigned | not null, default: 0  | Lender requires refundable security deposit                            |
...
| `is_insurance_recommended_tol`    | tinyint unsigned | not null, default: 0  | Flag for high-value tools ($1000+) where insurance is recommended      |
| `id_ftp_tol`                      | int              | default: null         | FK to fuel_type_ftp; NULL = tool does not use fuel                     |
...
- `idx_available_created_fee_tol` on `(is_available_tol, is_deleted_tol, created_at_tol, rental_fee_tol)`
...
- `idx_fuel_type_tol` on `id_ftp_tol`
- `idx_deleted_tol` on `is_deleted_tol`
- `idx_deleted_at_tol` on `deleted_at_tol`
- `fulltext_tool_search_tol` (FULLTEXT) on `(tool_name_tol, tool_description_tol)`

**SQL Constraints:**

```sql
CHECK (rental_fee_tol >= 0)
CHECK (default_deposit_amount_tol >= 0)
CHECK (estimated_value_tol IS NULL OR estimated_value_tol >= 0)
```

> **Note:**
>
> - `is_available_tol` = owner intent only.
> - True availability requires: `is_available_tol = 1` AND `is_deleted_tol = 0` AND no overlapping `availability_block_avb` AND no active `borrow_bor`.
> - Compute at query time (JOIN/NOT EXISTS) or via the `tool_availability_v` view for accuracy.
> - Trigger: BEFORE INSERT/UPDATE – reject if `id_acc_tol` references deleted account.
>
> **Soft-Delete Strategy:**
>
> - `is_deleted_tol = 1` is the single source of truth for soft-deleted tools — excluded from `available_tool_v`, `tool_availability_v`, and all listing views.
> - `deleted_at_tol` records WHEN deletion occurred; used by the 90-day deferred image cleanup job (image rows kept for cancellation/restore window before purge).
> - Restoration: clear both `is_deleted_tol = 0` and `deleted_at_tol = NULL`.
>
...
> - `idx_available_created_fee_tol` covers the public marketplace listing query (visible non-deleted tools, newest-first, with rental-fee sort/filter).
>
...
| `focal_x_tim`      | tinyint unsigned  | not null, default: 50 | Horizontal focal point 0-100 for CSS `object-position`                      |
| `focal_y_tim`      | tinyint unsigned  | not null, default: 50 | Vertical focal point 0-100 for CSS `object-position`                        |
| `width_tim`        | smallint unsigned | -                     | Intrinsic pixel width stored at upload time                                 |
...
#### notification_preference_ntp

Per-user opt-out preferences for notification types. Uses an opt-out model: no row means enabled; rows only exist for disabled types.

| Column            | Type      | Constraints                          | Notes                           |
|-------------------|-----------|--------------------------------------|---------------------------------|
| `id_ntp`          | int       | PK, auto-increment                   | -                               |
| `id_acc_ntp`      | int       | not null                             | FK to account_acc               |
| `id_ntt_ntp`      | int       | not null                             | FK to notification_type_ntt     |
| `is_enabled_ntp`  | boolean   | not null, default: true              | FALSE = user opted out          |
| `updated_at_ntp`  | timestamp | default: now(), on update: now()     | -                               |

**Indexes:**

- `uq_account_type_ntp` (UNIQUE) on `(id_acc_ntp, id_ntt_ntp)` — one preference per account per type

> **Opt-out model:** All notification types are enabled by default. A row is inserted only when a user explicitly disables a type. `ON DELETE CASCADE` on the account FK ensures cleanup on account deletion.
...
| `event_address_evt`     | varchar(255) | -                  | NULL for virtual events                    |
...
Junction table tracking event RSVPs.

| Column           | Type      | Constraints        | Notes              |
|------------------|-----------|--------------------|--------------------|
| `id_eya`         | int       | PK, auto-increment | -                  |
| `id_acc_eya`     | int       | not null           | FK to account_acc  |
| `id_evt_eya`     | int       | not null           | FK to event_evt    |
| `created_at_eya` | timestamp | default: now()     | -                  |

**Indexes:**

- `uq_attendee_event` (UNIQUE) on `(id_acc_eya, id_evt_eya)`
- `idx_event_attendee` on `id_evt_eya`
...
| `id_ptt_ptx`                  | tinyint unsigned | not null           | FK to payment_transaction_type_ptt          |
...
- `idx_txn_type_ptx` on `id_ptt_ptx`
...
> - Transaction type is normalized — references `payment_transaction_type_ptt` (replaces former `transaction_type_ptx VARCHAR` column).
...
| Accounts attend Events     | `account_acc`      | `event_evt`    | `event_attendee_eya`      |
...
| `account_acc`            | `avatar_vector_avv`  | `id_acc_avv`       | Admin uploads avatar vectors      |
| `avatar_vector_avv`      | `account_acc`        | `id_avv_acc`       | Account selects avatar vector     |
...
| `fuel_type_ftp`      | `tool_tol`        | `id_ftp_tol`   | Fuel/power source for tool     |
...
| `account_acc`       | `event_attendee_eya` | `id_acc_eya`        | Account RSVPs to events   |
| `event_evt`         | `event_attendee_eya` | `id_evt_eya`        | Event has attendees       |
...
| `payment_transaction_type_ptt` | `payment_transaction_ptx`      | `id_ptt_ptx`      | Transaction type (lookup)        |
...
|                                                 |                            |
|  +-------------------+    +---------------+    +-----------------+           |
|  | tool_condition_tcd|    | category_cat  |    | fuel_type_ftp   |           |
|  +--------+----------+    +------+--------+    +--------+--------+           |
|           |                      |                      |                    |
|           +----------+-----------+----------------------+                    |
|                      |                          |                            |
...
|                     v                                                        |
|           +----------------------+    +-----------------------------+        |
|           | notification_type_ntt|<---| notification_preference_ntp |        |
|           +----------------------+    +-----------------------------+        |
|                                                                              |
|  +-----------------+   +----------------------+   +-----------------+        |
|  |    event_evt    |   |  event_attendee_eya  |   | event_meta_evm  |        |
|  +-----------------+   +----------------------+   +-----------------+        |
|                                                                              |
+------------------------------------------------------------------------------+
...
|                                                                              |
|  +----------------------+                        +------------------+        |
|  | phpbb_integration_php|                        |  audit_log_aud   |        |
|  +----------------------+                        +--------+---------+        |
|                                                           |                  |
|                                                           v                  |
|                                                  +-----------------------+   |
...
|                         v                                                    |
|  +--------------------------------+    +-----------------------+             |
|  | payment_transaction_type_ptt   |--->| payment_transaction_  |             |
|  +--------------------------------+    | ptx                   |---> borrow_ |
|                                        +-----------+-----------+      bor    |
|                                                    |                         |
|                                                    v                         |
...
> Triggers enforce business rules, data integrity, and referential constraints at the database level. This section documents all 50 triggers in the schema.
...
| 13 | `trg_user_rating_after_insert` | AFTER INSERT | `user_rating_urt` | Refresh affected user's reputation snapshot |
| 14 | `trg_user_rating_after_update` | AFTER UPDATE | `user_rating_urt` | Refresh affected user's reputation snapshot |
| 15 | `trg_user_rating_after_delete` | AFTER DELETE | `user_rating_urt` | Refresh affected user's reputation snapshot |
| 16 | `trg_tool_rating_before_insert` | BEFORE INSERT | `tool_rating_trt` | Validate score range |
| 17 | `trg_dispute_before_insert` | BEFORE INSERT | `dispute_dsp` | Set default status to open |
| 18 | `trg_dispute_message_before_insert` | BEFORE INSERT | `dispute_message_dsm` | Set default message type to initial_report |
| 19 | `trg_borrow_waiver_before_insert` | BEFORE INSERT | `borrow_waiver_bwv` | Enforce required acknowledgments |
| 20 | `trg_handover_verification_before_insert` | BEFORE INSERT | `handover_verification_hov` | Auto-generate verification code and expiry |
| 21 | `trg_incident_report_before_insert` | BEFORE INSERT | `incident_report_irt` | Auto-calculate deadline compliance |
| 22 | `trg_tos_before_insert` | BEFORE INSERT | `terms_of_service_tos` | Deactivate previous ToS version |
| 23-50 | Lookup protection triggers | BEFORE UPDATE/DELETE | 14 lookup tables | Prevent modification of seeded lookup rows (28 triggers, 2 per table) |
...
#### trg_user_rating_after_insert

| Property | Value |
|----------|-------|
| **Timing** | AFTER INSERT |
| **Table** | `user_rating_urt` |
| **Purpose** | Refreshes the targeted user's reputation snapshot in `user_reputation_mat` immediately after a new rating is recorded |

**Business Rules:**

- Calls `sp_refresh_user_reputation_for(NEW.id_acc_target_urt)` so the rated user's materialized reputation reflects the new score without waiting for the 4-hour bulk refresh.

---

#### trg_user_rating_after_update

| Property | Value |
|----------|-------|
| **Timing** | AFTER UPDATE |
| **Table** | `user_rating_urt` |
| **Purpose** | Keeps both the prior and new target user's reputation snapshots in sync after a rating is edited |

**Business Rules:**

- Calls `sp_refresh_user_reputation_for(NEW.id_acc_target_urt)`.
- If the target changed (`OLD.id_acc_target_urt != NEW.id_acc_target_urt`), also refreshes the previous target so its aggregates lose the moved rating.

---

#### trg_user_rating_after_delete

| Property | Value |
|----------|-------|
| **Timing** | AFTER DELETE |
| **Table** | `user_rating_urt` |
| **Purpose** | Refreshes the targeted user's reputation snapshot when a rating is removed |

**Business Rules:**

- Calls `sp_refresh_user_reputation_for(OLD.id_acc_target_urt)` so the deleted rating drops out of the user's averages and counts.
...
Lookup table protection triggers prevent deletion or renaming of system-required values that business logic triggers, stored procedures, and functions depend on. All 28 triggers follow the same pattern: a BEFORE DELETE trigger that rejects deletion of protected values, and a BEFORE UPDATE trigger that rejects renaming of protected values.

| Table | Protected Values | DELETE Trigger | UPDATE Trigger |
|-------|-----------------|----------------|----------------|
| `role_rol` | member, admin, super_admin | `trg_role_before_delete` | `trg_role_before_update` |
| `account_status_ast` | pending, active, suspended, deleted | `trg_account_status_before_delete` | `trg_account_status_before_update` |
| `tool_condition_tcd` | new, good, fair, poor | `trg_tool_condition_before_delete` | `trg_tool_condition_before_update` |
...
| `dispute_status_dst` | open, resolved, dismissed | `trg_dispute_status_before_delete` | `trg_dispute_status_before_update` |
| `dispute_message_type_dmt` | initial_report, response, admin_note, resolution | `trg_dispute_message_type_before_delete` | `trg_dispute_message_type_before_update` |
| `notification_type_ntt` | request, approval, due, return, rating, denial, role_change | `trg_notification_type_before_delete` | `trg_notification_type_before_update` |
| `waiver_type_wtp` | borrow_waiver, condition_acknowledgment, liability_release | `trg_waiver_type_before_delete` | `trg_waiver_type_before_update` |
| `handover_type_hot` | pickup, return | `trg_handover_type_before_delete` | `trg_handover_type_before_update` |
| `incident_type_ity` | damage, theft, loss, injury, late_return, condition_dispute, other | `trg_incident_type_before_delete` | `trg_incident_type_before_update` |
...
| `payment_provider_ppv` | stripe, paypal, manual | `trg_payment_provider_before_delete` | `trg_payment_provider_before_update` |
...
> Views provide pre-built queries for common application needs. This section documents all 27 views in the schema.
...
| 7 | `tool_availability_v` | Single-row availability status per tool (UNLISTED / BORROWED / PENDING / BLOCKED / AVAILABLE) | `tool_tol`, `borrow_bor`, `availability_block_avb` |
| 8 | `tool_detail_v` | Full tool listing with availability status | `tool_tol`, `account_acc`, `tool_rating_trt`, `borrow_bor`, `category_cat` |
| 9 | `user_reputation_v` | Live reputation scores (computed) | `account_acc`, `user_rating_urt`, `tool_tol`, `borrow_bor` |
| 10 | `tool_statistics_v` | Live tool usage statistics (computed) | `tool_tol`, `tool_rating_trt`, `borrow_bor`, `incident_report_irt` |
| 11 | `neighborhood_summary_v` | Live neighborhood statistics (computed) | `neighborhood_nbh`, `account_acc`, `tool_tol`, `borrow_bor`, `event_evt` |
| 12 | `open_dispute_v` | Active disputes requiring attention | `dispute_dsp`, `borrow_bor`, `account_acc`, `dispute_message_dsm` |
| 13 | `pending_deposit_v` | Deposits in pending/held status | `security_deposit_sdp`, `borrow_bor`, `account_acc`, `payment_provider_ppv` |
| 14 | `current_tos_v` | Currently active Terms of Service | `terms_of_service_tos` |
| 15 | `tos_acceptance_required_v` | Users who need to accept current ToS | `account_acc`, `terms_of_service_tos`, `tos_acceptance_tac` |
| 16 | `pending_waiver_v` | Approved borrows missing waivers | `borrow_bor`, `tool_tol`, `account_acc`, `borrow_waiver_bwv` |
| 17 | `open_incident_v` | Unresolved incident reports | `incident_report_irt`, `borrow_bor`, `tool_tol`, `account_acc`, `incident_photo_iph` |
| 18 | `pending_handover_v` | Handover verifications awaiting completion | `handover_verification_hov`, `borrow_bor`, `tool_tol`, `account_acc` |
| 19 | `unread_notification_v` | Unread notifications per user | `notification_ntf`, `notification_type_ntt` |
| 20 | `user_bookmarks_v` | User bookmark list with tool details | `tool_bookmark_acctol`, `tool_tol`, `account_acc`, `tool_image_tim` |
| 21 | `category_summary_v` | Live category statistics (computed) | `category_cat`, `tool_category_tolcat`, `tool_tol`, `tool_rating_trt`, `borrow_bor` |
| 22 | `upcoming_event_v` | Future community events | `event_evt`, `neighborhood_nbh`, `account_acc` |
| 23 | `payment_history_v` | Payment transaction history with provider and participant names | `payment_transaction_ptx`, `payment_provider_ppv`, `payment_transaction_type_ptt`, `borrow_bor`, `tool_tol`, `account_acc` |
| 24 | `neighborhood_summary_fast_v` | Fast dashboard read from materialized table | `neighborhood_summary_mat` |
| 25 | `user_reputation_fast_v` | Fast profile read from materialized table | `user_reputation_mat` |
| 26 | `tool_statistics_fast_v` | Fast analytics read from materialized table | `tool_statistics_mat` |
| 27 | `category_summary_fast_v` | Fast browse page read from materialized table | `category_summary_mat` |
...
| Property | Value |
|----------|-------|
| **Purpose** | Returns a single-row availability classification per tool — used by listing pages, search, and the borrow request flow to make a fast, consistent availability decision without re-implementing the rules at every call site |
| **Tables** | `tool_tol`, `borrow_bor`, `availability_block_avb` (via `fn_get_borrow_status_id` for status name → ID resolution) |
| **Filter** | `is_deleted_tol = 0` (soft-deleted tools excluded) |
| **Use Case** | Marketplace listings; borrow-request validation; "Add to bookmarks" checks |

**Key Columns:**

| Column                | Source          | Notes                                                                |
|-----------------------|-----------------|----------------------------------------------------------------------|
| `id_tol`              | `tool_tol`      | Tool primary key                                                     |
| `availability_status` | CASE expression | One of: `UNLISTED`, `BORROWED`, `PENDING`, `BLOCKED`, `AVAILABLE`    |

**Availability Status Logic (in priority order):**

1. `UNLISTED` — `is_available_tol = 0` (owner toggled the tool off)
2. `BORROWED` — there exists a `borrow_bor` row in `borrowed` status for this tool
3. `PENDING` — there exists a `borrow_bor` row in `requested` or `approved` status for this tool
4. `BLOCKED` — `NOW()` falls within an active `availability_block_avb` window
5. `AVAILABLE` — none of the above

> **Note:**
>
> - The first matching condition wins; statuses are mutually exclusive.
> - Soft-deleted tools (`is_deleted_tol = 1`) never appear in this view.
> - Status name lookups are resolved via `fn_get_borrow_status_id()` so the view stays correct even if lookup IDs are reseeded.
...
| Property | Value |
|----------|-------|
| **Purpose** | Payment transaction history with provider, tool, and participant names |
| **Tables** | `payment_transaction_ptx`, `payment_provider_ppv`, `payment_transaction_type_ptt`, `borrow_bor`, `tool_tol`, `account_acc` (x2 for from/to accounts) |
| **Filter** | None (shows all transactions) |
| **Use Case** | Admin payment audit; user transaction history |

**Key Columns:** `transaction_type` (joined from `payment_transaction_type_ptt`), `amount_ptx`, `external_transaction_id_ptx`, `external_status_ptx`, `payment_provider`, `tool_name_tol`, `from_name`, `to_name`
...
> Stored procedures encapsulate complex business logic and multi-step operations. This section documents all 31 stored procedures in the schema.
...
| 4 | `sp_refresh_user_reputation` | Materialized Refresh | Rebuild `user_reputation_mat` cache (all users) |
| 5 | `sp_refresh_user_reputation_for` | Materialized Refresh | Rebuild a single user's row in `user_reputation_mat` (called by rating triggers) |
| 6 | `sp_refresh_tool_statistics` | Materialized Refresh | Rebuild `tool_statistics_mat` cache |
| 7 | `sp_refresh_category_summary` | Materialized Refresh | Rebuild `category_summary_mat` cache |
| 8 | `sp_refresh_platform_daily_stat` | Materialized Refresh | Rebuild `platform_daily_stat_pds` for current day |
| 9 | `sp_refresh_all_summaries` | Materialized Refresh | Master procedure — calls all 5 bulk refresh procedures |
| 10 | `sp_create_borrow_request` | Borrow Workflow | Validate availability and create borrow request |
| 11 | `sp_approve_borrow_request` | Borrow Workflow | Approve pending request, generate handover code |
| 12 | `sp_deny_borrow_request` | Borrow Workflow | Deny pending request with reason |
| 13 | `sp_complete_pickup` | Borrow Workflow | Record tool pickup via handover verification |
| 14 | `sp_complete_return` | Borrow Workflow | Record tool return via handover verification |
| 15 | `sp_cancel_borrow_request` | Borrow Workflow | Cancel request (borrower-initiated) |
| 16 | `sp_rate_user` | Rating | Submit a user-to-user rating after borrow |
| 17 | `sp_rate_tool` | Rating | Submit a tool rating after borrow |
| 18 | `sp_send_notification` | Notification | Create a notification for a specific account |
| 19 | `sp_mark_notifications_read` | Notification | Bulk-mark notifications as read for an account |
| 20 | `sp_clear_read_notifications` | Notification | Delete all read notifications for an account |
| 21 | `sp_send_overdue_notifications` | Notification | Batch-send overdue alerts to borrowers and lenders |
| 22 | `sp_process_stale_approved_borrows` | Borrow Expiration | Warn at 48h and auto-cancel approved borrows after 72h without pickup |
| 23 | `sp_cleanup_expired_handover_codes` | Maintenance | Expire handover codes past their expiry window |
| 24 | `sp_archive_old_notifications` | Maintenance | Archive read notifications older than threshold |
| 25 | `sp_cleanup_old_search_logs` | Maintenance | Purge search logs older than retention period |
| 26 | `sp_release_deposit_on_return` | Deposit | Release held security deposit on borrow return |
| 27 | `sp_forfeit_deposit` | Deposit | Forfeit held deposit with amount, reason, and optional incident link |
| 28 | `sp_search_available_tools` | Search & Query | Full-text search of available tools with filters |
| 29 | `sp_get_user_borrow_history` | Search & Query | Paginated borrow history for a specific account |
| 30 | `sp_create_bookmark` | Bookmark | Idempotently bookmark a tool for an account |
| 31 | `sp_delete_bookmark` | Bookmark | Remove a tool bookmark for an account |
...
#### `sp_refresh_user_reputation_for`

Targeted, single-user variant of `sp_refresh_user_reputation`. Recomputes one row in `user_reputation_mat` instead of rebuilding the whole table — used by the `user_rating_urt` AFTER triggers so a freshly recorded rating is immediately reflected on the affected user's profile.

| Property | Value |
|----------|-------|
| **Target Table** | `user_reputation_mat` (single row) |
| **Source Tables** | Same as `sp_refresh_user_reputation` |
| **Refresh Strategy** | DELETE + INSERT for the single `id_acc` argument |
| **Called By** | `trg_user_rating_after_insert`, `trg_user_rating_after_update`, `trg_user_rating_after_delete` |

**Parameters:**

| Parameter      | Direction | Type  | Description                                     |
|----------------|-----------|-------|-------------------------------------------------|
| `p_account_id` | `IN`      | `INT` | Account whose reputation row should be rebuilt  |

> **Note:**
>
> - No-ops if the account does not exist or has `deleted` status.
> - Keeps the materialized cache consistent without waiting for the 4-hour bulk refresh event.
...
#### `sp_clear_read_notifications`

Deletes all read notifications for a specific account. Provides a user-initiated cleanup action from the notifications page.

| Property | Value |
|----------|-------|
| **Category** | Notification |
| **Transaction** | No (single DELETE) |
...
|-----------|-----------|------|-------------|
| `p_account_id` | `IN` | `INT` | Target account `id_acc` |
| `p_count` | `OUT` | `INT` | Number of notifications deleted |

**Filter:** Deletes where `id_acc_ntf = p_account_id` AND `is_read_ntf = TRUE`

**Usage:**

```sql
CALL sp_clear_read_notifications(2, @count);
```
...
### 11.6 Borrow Expiration Procedures

#### `sp_process_stale_approved_borrows`

Automatically warns borrowers at 48 hours and cancels approved borrow requests that have not completed pickup within 72 hours. Cleans up pending deposits and notifies both borrower and lender.

> **Renamed in phase 6.** Previously named `sp_expire_stale_approved_borrows`. The companion event `evt_expire_stale_approved_borrows` retained its original name and was updated to call this procedure.

| Property | Value |
|----------|-------|
| **Category** | Borrow Expiration |
| **Transaction** | No (uses temp table for batch operations) |
| **Tables Modified** | `notification_ntf` (INSERT), `borrow_bor` (UPDATE), `security_deposit_sdp` (DELETE) |
| **Called By** | `evt_expire_stale_approved_borrows` (hourly event) |

**Parameters:**

| Parameter | Direction | Type | Description |
|-----------|-----------|------|-------------|
| `p_warned` | `OUT` | `INT` | Number of 48-hour warning notifications sent |
| `p_expired` | `OUT` | `INT` | Number of borrows auto-cancelled |

**Logic:**

1. Find approved borrows between 48–72 hours old that haven't been warned — send pickup expiring notification to borrower
2. Find approved borrows older than 72 hours — cancel them with `[AUTO-CANCELLED]` note
3. Delete any pending security deposits for cancelled borrows
4. Notify both borrower (request expired) and lender (tool available again)

**Deduplication:** Checks `NOT EXISTS` on notifications with title `LIKE 'Pickup Expiring:%'` to prevent duplicate warnings.

**Usage:**

```sql
CALL sp_process_stale_approved_borrows(@warned, @expired);
```
...
Transitions a held deposit to forfeited status, recording the forfeiture amount, reason, and optional incident link. Validates that the deposit exists, is in held status, and that the forfeiture amount does not exceed the deposit amount.
...
| **Transaction** | Yes — `START TRANSACTION` / `COMMIT` with `ROLLBACK` on error |
| **Row Locking** | `SELECT ... FOR UPDATE` on `security_deposit_sdp` |
| **Tables Modified** | `security_deposit_sdp` (UPDATE — status, `forfeited_at_sdp`, `forfeited_amount_sdp`, `forfeiture_reason_sdp`, `id_irt_sdp`) |

**Parameters:**

| Parameter | Direction | Type | Description |
|-----------|-----------|------|-------------|
| `p_deposit_id` | `IN` | `INT` | Security deposit to forfeit |
| `p_amount` | `IN` | `DECIMAL(8,2)` | Forfeiture amount (must be > 0 and <= deposit amount) |
| `p_reason` | `IN` | `TEXT` | Reason for forfeiture |
| `p_incident_id` | `IN` | `INT` | Optional incident report link (NULL if not incident-related) |
| `p_success` | `OUT` | `BOOLEAN` | `TRUE` on success |
| `p_error_message` | `OUT` | `VARCHAR(255)` | Error description on failure |

**Logic Steps:**

1. Look up the deposit (`FOR UPDATE`)
2. If not found → error
3. If not in `held` status → error
4. If amount <= 0 or amount > deposit amount → error
5. Update status to `forfeited`, set `forfeited_at_sdp`, `forfeited_amount_sdp`, `forfeiture_reason_sdp`, and `id_irt_sdp`

**Usage:**

```sql
CALL sp_forfeit_deposit(1, 50.00, 'Tool returned with damage', NULL, @success, @error);
```

### 11.9 Search & Query Procedures
...
### 11.10 Bookmark Procedures

#### `sp_create_bookmark`

Bookmarks a tool for an account. Idempotent — re-bookmarking the same tool is a no-op rather than an error, so the application can safely call this from a client-side toggle without coordinating against existing state.

| Property | Value |
|----------|-------|
| **Category** | Bookmark |
| **Transaction** | Single-statement (atomic) |
| **Tables Modified** | `tool_bookmark_acctol` (INSERT … ON DUPLICATE KEY UPDATE) |

**Parameters:**

| Parameter      | Direction | Type  | Description                            |
|----------------|-----------|-------|----------------------------------------|
| `p_account_id` | `IN`      | `INT` | Account creating the bookmark          |
| `p_tool_id`    | `IN`      | `INT` | Tool being bookmarked                  |

> **Note:**
>
> - The `(id_acc_acctol, id_tol_acctol)` UNIQUE index is what makes this idempotent.
> - Caller is responsible for verifying the account and tool exist; this procedure does not validate either explicitly (FK constraints will reject orphaned references).

#### `sp_delete_bookmark`

Removes a bookmark. Silent if no matching row exists, so the call is safe even when state has drifted (e.g., the user un-bookmarks twice from different tabs).

| Property | Value |
|----------|-------|
| **Category** | Bookmark |
| **Transaction** | Single-statement (atomic) |
| **Tables Modified** | `tool_bookmark_acctol` (DELETE) |

**Parameters:**

| Parameter      | Direction | Type  | Description                            |
|----------------|-----------|-------|----------------------------------------|
| `p_account_id` | `IN`      | `INT` | Account removing the bookmark          |
| `p_tool_id`    | `IN`      | `INT` | Tool being un-bookmarked               |
...
> Scheduled events automate recurring maintenance and data refresh operations. This section documents all 9 scheduled events in the schema.
...
| `evt_expire_stale_approved_borrows` | Every 1 hour | `CURRENT_TIMESTAMP` | `sp_process_stale_approved_borrows()` | Auto-cancel approved borrows after 72h without pickup |
...
### 13.3 Borrow Expiration Events

#### `evt_expire_stale_approved_borrows`

Warns borrowers at 48 hours and auto-cancels approved borrow requests after 72 hours without pickup. Runs hourly for timely enforcement.
...
| **Starts** | `CURRENT_TIMESTAMP` |
| **Status** | `ENABLE` |
| **Procedure Called** | `sp_process_stale_approved_borrows()` (renamed from `sp_expire_stale_approved_borrows` in phase 6) |
| **Target Tables** | `notification_ntf` (INSERT), `borrow_bor` (UPDATE), `security_deposit_sdp` (DELETE) |

> **Two-phase enforcement:** At 48 hours, a warning notification is sent. At 72 hours, the borrow is cancelled, pending deposits are deleted, and both parties are notified. Deduplication prevents repeat warnings via `NOT EXISTS` on notification titles.

### 13.4 Maintenance Events
...
> The dump file seeds all lookup tables and geographic reference data. No sample accounts, tools, borrows, or other transactional data are included.

The dump file seeds 18 lookup tables plus geographic reference data for the Asheville/Hendersonville, NC service area (26 ZIP codes, 67 neighborhoods, 67 neighborhood-ZIP associations). All seed data is wrapped in a single `START TRANSACTION` / `COMMIT` block for atomic insertion.

### 14.1 Required Lookup Data

These 18 lookup tables **must** be seeded before any other data can be inserted (foreign key dependencies). Lookup table protection triggers prevent modification of seeded rows.
...
| `fuel_type_ftp` | `gasoline`, `diesel`, `propane`, `two-stroke mix`, `electric/battery`, `kerosene`, `natural gas` |
...
| `notification_type_ntt` | `request`, `approval`, `due`, `return`, `rating`, `denial`, `role_change` |
...
| `payment_transaction_type_ptt` | `deposit_hold`, `rental_fee`, `deposit_release`, `deposit_forfeit`, `refund` |
...
The geographic seed data covers the **Asheville and Hendersonville, NC** metropolitan area.

#### ZIP Codes (26 entries)

| Area | ZIP Codes |
| --- | --- |
| Asheville (Buncombe County core) | `28801` (Downtown), `28803` (South), `28804` (North), `28805` (East), `28806` (West) |
| Greater Buncombe | `28704` (Arden), `28711` (Black Mountain), `28715` (Candler), `28728` (Enka/Candler), `28778` (Swannanoa), `28787` (Weaverville) |
| Hendersonville (Henderson County core) | `28739` (Hendersonville/Laurel Park), `28791` (Hendersonville North), `28792` (Hendersonville East) |
| Henderson County area | `28724` (Dana), `28726` (East Flat Rock), `28727` (Edneyville), `28729` (Etowah), `28731` (Flat Rock), `28732` (Fletcher), `28735` (Gerton), `28742` (Horse Shoe), `28758` (Mountain Home), `28759` (Mills River), `28784` (Tuxedo), `28790` (Zirconia) |

Each ZIP code includes latitude, longitude, and a spatial `POINT` column using `ST_GeomFromText()` with SRID 4326 (WGS84) and `axis-order=long-lat`.

#### Neighborhoods (67 entries)

| Region | Count | Examples |
| --- | --- | --- |
| Buncombe County (Asheville & surrounds) | 38 | Albemarle Park, Arden, Beaver Lake, Beaverdam Valley, Beverly Hills, Biltmore Forest, Biltmore Park, Biltmore Village, Black Mountain, Candler, Chestnut Hills, Chunn's Cove, Downtown Asheville, East Asheville, Enka, Falconhurst, Five Points, Grace, Grove Park, Haw Creek, Kenilworth, Lakeview Park, Malvern Hills, Montford, North Asheville, Norwood Park, Oakley, River Arts District, Royal Pines, Shiloh, South Asheville, South Slope, Sulphur Springs, Swannanoa, Town Mountain, Weaverville, West Asheville, Woodfin |
| Henderson County (Hendersonville & surrounds) | 29 | Balfour, Barker Heights, Carriage Park, Champion Hills, Cummings Cove, Dana, Downtown Hendersonville, Druid Hills, East Flat Rock, Edneyville, Etowah, Fifth Avenue West, Flat Rock, Fletcher, Fruitland, Gerton, Green River, Haywood Knolls, Hoopers Creek, Horse Shoe, Hyman Heights, Kenmure, Laurel Park, Mills River, Mountain Home, Osceola Lake, Tuxedo, Valley Hill, Zirconia |

> **Note:** Each neighborhood includes city, state FK, latitude, longitude, and a spatial `POINT` column for proximity queries. "Town Mountain" appears only in the Buncombe set (it spans counties but shares ZIP 28804).

#### Neighborhood-ZIP Associations (67 entries)

Each neighborhood is linked to one or more ZIP codes via `neighborhood_zip_nbhzpc`. The `is_primary_nbhzpc` flag designates the main neighborhood for each ZIP code (the BEFORE INSERT/UPDATE triggers enforce single-primary). Examples:

- `28801` — Downtown Asheville is primary; Albemarle Park, Chestnut Hills, Five Points, Montford, River Arts District, and South Slope are secondary
- `28803` — South Asheville is primary; Biltmore Forest, Biltmore Park, Biltmore Village, Kenilworth, Oakley, and Shiloh are secondary
- `28804` — North Asheville is primary; Beaver Lake, Beaverdam Valley, Grace, Grove Park, Lakeview Park, Norwood Park, and Town Mountain are secondary
- `28704` — Arden is primary; Royal Pines is secondary
- `28739`/`28791`/`28792` — Hendersonville core ZIPs hosting Downtown Hendersonville, Druid Hills, Hyman Heights, Laurel Park, etc.
