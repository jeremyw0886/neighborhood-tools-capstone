...
return DASHBOARD_RE.test(path);}catch{return false;}};
...
fetchPage(link.href).catch(()=>{});},PREFETCH_DELAY);};
const cancelPrefetch=()=>{
clearTimeout(hoverTimer);
hoverTimer=null;};
mainEl.addEventListener('pointerenter',(e)=>{
const link=e.target.closest('a[href]');
if(link)startPrefetch(link);},true);
mainEl.addEventListener('pointerleave',(e)=>{
if(e.target.closest('a[href]'))cancelPrefetch();},true);
...
startY=e.touches[0].clientY;},{passive:true});
...
const backLink=mainEl.querySelector('nav[aria-label="Back"] a[href]');
if(!backLink||!shouldIntercept(backLink))return;
navigate(backLink.href,'back');},{passive:true});};
