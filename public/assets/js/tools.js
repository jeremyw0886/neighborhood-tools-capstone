...
class ImageCrop {
  ...
  static init() {
    ...
    NT.crop = {
      ...
      openReposition: (src, fx, fy, id) => instance.#openReposition(src, fx, fy, id),
...
class PhotoQueue {
  ...
  constructor(queue, dropZone, fileInput, dataContainer) {
    ...
    if (NT.crop) {
      NT.crop.onConfirm((mode, data) => {
        if (mode === 'upload') {
          this.#queuedPhotos.push({ file: data.file, focalX: data.focalX, focalY: data.focalY });
          NT.crop.close();
          this.#renderQueue();
  ...
  #renderQueue() {
    ...
    for (let i = 0; i < this.#queuedPhotos.length; i++) {
      ...
      img.src = blobUrl;
      img.alt = `Preview of ${entry.file.name}`;
      img.width = 400;
      img.height = 268;
      img.decoding = 'async';
      ...
      li.append(img, repoDiv, removeDiv);
