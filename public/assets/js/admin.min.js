...
static#instance=null;#abortController=new AbortController();#pending=false;
constructor(){
...
return AdminDeleteConfirm.init();}#handleSubmit=async(e)=>{
const form=e.target;
...
static#instance=null;#dialog=document.querySelector('[data-role-confirm]');#message=this.#dialog?.querySelector('[data-role-confirm-message]');#pendingForm=null;#pendingSelect=null;#abortController=new AbortController();
constructor(){
...
return RoleChangeConfirm.init();}#handleSubmit=(e)=>{
const form=e.target;
...
this.#dialog.showModal();};#handleClose=()=>{
if(this.#dialog.returnValue==='confirm'&&this.#pendingForm){
this.#pendingForm.submit();}else if(this.#pendingSelect){
...
return DeleteUserConfirm.init();}#handleSubmit=async(e)=>{
const form=e.target;
...
class PurgeConfirm{
static#instance=null;#dialog=document.querySelector('[data-purge-confirm]');#form=this.#dialog?.querySelector('[data-purge-form]');#nameDisplay=this.#dialog?.querySelector('[data-purge-expected-name]');#nameInput=this.#dialog?.querySelector('[data-purge-name-input]');#submitBtn=this.#dialog?.querySelector('[data-purge-submit]');#cancelBtn=this.#dialog?.querySelector('[data-purge-cancel]');#expectedName='';#abortController=new AbortController();
constructor(){
const opts={signal:this.#abortController.signal};
...
return PurgeConfirm.init();}#handleTrigger=(e)=>{
...
this.#dialog.showModal();};#handleInput=()=>{
this.#submitBtn.disabled=
this.#nameInput.value.trim().toLowerCase()!==this.#expectedName.toLowerCase();};#handleCancel=()=>{
this.#dialog.close();
this.#nameInput.value='';
this.#submitBtn.disabled=true;};}
PurgeConfirm.init();
...
static#REDUCED_MOTION=matchMedia('(prefers-reduced-motion: reduce)');#content=document.querySelector('[data-admin-content]');#header=document.querySelector('[data-admin-header]');#section=document.querySelector('main > section[aria-labelledby]');#nav=document.querySelector('[data-admin-body] > nav');#abortController=null;#prefetchCache=new Map();#ac=new AbortController();#navIndex=0;#loadedCss=new Set(
...
return(AdminRouter.#instance=new AdminRouter());}#handleClick=(e)=>{
...
e.preventDefault();
this.#navigateTo(url.href);};#handleSubmit=(e)=>{
const form=e.target;
...
this.#navigateTo(url.href);};#handlePopstate=(e)=>{
if(AdminRouter.#ADMIN_PATTERN.test(location.pathname)){
this.#navIndex=e.state?.idx??0;
this.#navigateTo(location.href,false);}};#handleHover=(e)=>{
const link=e.target.closest?.('a[href]');
if(!link)return;
const url=new URL(link.href,location.origin);
if(url.origin!==location.origin)return;
...
html=cached.html;
({heading,icon,description,sectionId,title,css,js}=cached);}else{
...
if(newContent){
html=newContent.innerHTML;}else{
location.href=url;
return;}}
...
this.#content.removeAttribute('data-transition');},{once:true});}catch(err){
...
if(headingSpan){
headingSpan.textContent=heading;}else{
...
this.#header.appendChild(newP);}}else if(p){
p.hidden=true;}}#updatePageTitle(title){
...
document.head.appendChild(script);});}}#updateNavActiveState(url){
if(!this.#nav)return;
