...
#li;#onConfirm;#swipeId;#state=null;#abortController=new AbortController();
...
this.#li.classList.remove('swiping');}#articleSelector(){
return`section[aria-labelledby="notifications-heading"] > ol > li[data-swipe-id="${this.#swipeId}"] > article`;}#ensureTray(){
...
this.#li.appendChild(tray);}#snapTranslate(x,callback){
...
setTimeout(done,300);}}#reset(){
this.#snapTranslate(0,()=>{
NT.style.removeRule(`swipe-${this.#swipeId}`);
this.#li.querySelector('[data-swipe-tray]')?.remove();
this.#li.classList.remove('swiping');});}#confirm(){
...
NT.style.removeRule(`swipe-${this.#swipeId}`);
this.#li.querySelector('[data-swipe-tray]')?.remove();
this.#li.classList.remove('swiping');});}#handleTouchStart=(e)=>{
...
this.#li.classList.add('swiping');}else{
return;}}
e.preventDefault();
...
`transform:translateX(${clampedX}px);transition:none`);};#handleTouchEnd=()=>{
...
static#REDUCED_MOTION=matchMedia('(prefers-reduced-motion: reduce)');
#section;
#swipeInstances=[];#removalIdCounter=0;
#filterController=null;#abortController=new AbortController();
constructor(section){
...
const section=document.querySelector('section[aria-labelledby="notifications-heading"]');
...
this.#swipeInstances=[];}
#getBellLink(){
return document.querySelector('#bell-wrapper > a[href="/notifications"]');}#readBadgeCount(){
const badge=this.#getBellLink()?.querySelector('span');
return badge?parseInt(badge.textContent,10)||0:0;}#setBadgeCount(count){
...
const filterNav=this.#section.querySelector('nav[aria-label="Filter notifications"]');
filterNav?.setAttribute('aria-busy','true');
try{
const res=await NT.fetch(url,{signal:this.#filterController.signal});
const html=await res.text();
const doc=new DOMParser().parseFromString(html,'text/html');
const fresh=doc.querySelector('section[aria-labelledby="notifications-heading"]');
...
this.#createSwipeInstances();}catch(err){
...
if(!form)return;
const markReadForm=form.closest('li[data-unread] > article > div > footer > form[action$="/read"]');
if(markReadForm){
e.preventDefault();
this.#handleMarkRead(markReadForm);
return;}
const deleteForm=form.closest('li > article > div > footer > form[action$="/delete"]');
if(deleteForm){
e.preventDefault();
...
if(clearReadForm){
e.preventDefault();
this.#handleClearRead(clearReadForm);}};#handleClick=(e)=>{
const filterLink=e.target.closest('nav[aria-label="Filter notifications"] a[href]');
const paginationLink=e.target.closest('nav[aria-label="Pagination"] a[href]');
const link=filterLink||paginationLink;
if(!link)return;
e.preventDefault();
this.#swapContent(link.href);};#handlePopstate=()=>{
