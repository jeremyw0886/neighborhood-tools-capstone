...
if(fx&&fy){
setRule(key,`#${CSS.escape(img.id)}`,`object-position:${fx}% ${fy}%`);}else{
removeRule(key);}}}
const FOCUSABLE=['a[href]','button:not([disabled])','input:not([disabled]):not([type="hidden"])','select:not([disabled])','textarea:not([disabled])','[tabindex]:not([tabindex="-1"])',].join(',');
...
btn.removeAttribute('data-visible');}else{
btn.setAttribute('data-visible','');}},{rootMargin:'0px'});
...
observeLazyImage(node);}else{
for(const img of node.querySelectorAll('img[loading="lazy"]')){
observeLazyImage(img);}}}}}).observe(document.body,{childList:true,subtree:true});}
...
hideDropdown();
if(config.onSelect){
config.onSelect(item,form);}else{
...
renderSuggestions(items);}catch(err){
...
return;}
currentItems=items;
dropdown.replaceChildren(...items.map((item,i)=>{
const li=document.createElement('li');
li.setAttribute('role','option');
li.id=`${listboxId}-${i}`;
if(config.renderItem){
config.renderItem(li,item);}else{
...
setActiveIndex(activeIndex<count-1?activeIndex+1:0);}else if(e.key==='ArrowUp'){
e.preventDefault();
if(dropdown.hidden||!count)return;
setActiveIndex(activeIndex>0?activeIndex-1:count-1);}else if(e.key==='Enter'&&activeIndex>=0&&activeIndex<count){
e.preventDefault();
selectOption(currentItems[activeIndex]);}else if(e.key==='Escape'&&!dropdown.hidden){
e.preventDefault();
hideDropdown();}}
