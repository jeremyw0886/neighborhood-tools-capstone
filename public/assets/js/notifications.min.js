...
NT.style.setRule(
key,
selector,'overflow: hidden; max-height: 0; opacity: 0; transition: max-height 0.3s ease, opacity 0.3s ease');
const done=()=>{
...
swipeState.li.classList.add('swiping');}else{
return;}}
e.preventDefault();
...
`swipe-${id}`,
selector,
`transform: translateX(${clampedX}px); transition: none`);},{passive:false});
