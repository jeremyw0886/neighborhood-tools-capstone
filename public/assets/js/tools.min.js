...
currentPagination.outerHTML=NT.sanitizeHtml(data.paginationHtml);}else{
...
static teardown(){
PhotoQueue.#instance?.destroy();}#bind(){
const{signal}=this.#abortController;
...
return primary;}#handleQueueKeydown=(e)=>{
const li=e.target.closest('li[data-queue-index]');
...
static teardown(){
GalleryManager.#instance?.destroy();}#setBusy(state){
this.#busy=state;
...
.map((li)=>parseInt(li.dataset.imageId,10));}#ensureGallery(){
...
const li=this.#buildGalleryItem({...img,
...
NT.toast('Failed to save position.','error');
NT.crop.setConfirmState(false,false);}finally{
this.#setBusy(false);}}#handleDragstart=(e)=>{
...
li.dataset.dragging='';
e.dataTransfer.effectAllowed='move';
e.dataTransfer.setData('text/plain',li.dataset.imageId);};#handleDragend=(e)=>{
const li=e.target.closest('li[data-image-id]');
if(li)delete li.dataset.dragging;
this.#dragItem=null;
for(const el of e.currentTarget.querySelectorAll('[data-drag-over]')){
delete el.dataset.dragOver;}};#handleDragover=(e)=>{
e.preventDefault();
e.dataTransfer.dropEffect='move';
...
for(const el of e.currentTarget.querySelectorAll('[data-drag-over]')){
delete el.dataset.dragOver;}
target.dataset.dragOver='';};#handleDrop=async(e)=>{
e.preventDefault();
...
this.#setBusy(false);};#handleKeydown=async(e)=>{
...
li.focus();}
this.#setBusy(false);};#handleClick=async(e)=>{
...
NT.crop?.openReposition(img.src,fx,fy,imageId);}};#handleChange=async(e)=>{
const radio=e.target.closest('[data-primary-radio]');
if(!radio||this.#busy)return;
await this.#handlePrimaryChange(e.currentTarget,radio);};#handleBlur=async(e)=>{
...
static teardown(){
GalleryViewer.#instance?.destroy();}#setActiveThumb(index){
...
this.#mainSourceAvif.hidden=false;}else{
