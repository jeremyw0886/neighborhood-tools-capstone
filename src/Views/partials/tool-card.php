  ...
  <article
    ...
    <div>
      ...
      <footer>
        ...
        <?php endif; ?>
        <?php
        $ownerVec = $tool['owner_vector_avatar'] ?? null;
        $ownerPhoto = $tool['owner_avatar'] ?? null;
        $ownerName = $tool['owner_name'] ?? 'Owner';
        $isRasterOwnerAvatar = !\App\Core\ViewHelper::isVectorAvatar($ownerVec) && !empty($ownerPhoto);
        ?>
        <?php if ($isRasterOwnerAvatar):
          $avatarVariants = \App\Core\ImageProcessor::getAvailableVariants(
            $ownerPhoto,
            fullWidth: (int) ($tool['owner_avatar_width'] ?? 0) ?: null,
            widths: [80, 150],
            uploadDir: 'profiles',
          );
          $avatarSrcsets = \App\Core\ImageProcessor::buildSrcset($avatarVariants, '/uploads/profiles/');
          $avatarIsWebp = str_ends_with($ownerPhoto, '.webp');
        ?>
          <picture>
        ...
        <?php else: ?>
          <img src="<?= htmlspecialchars(\App\Core\ViewHelper::avatarUrl($ownerVec, $ownerPhoto)) ?>"
            alt="<?= htmlspecialchars($ownerName) ?>"
            width="28" height="28"
            loading="lazy"
            decoding="async">
