...
static#instance=null;#abortController=new AbortController();
constructor(nav){
...
nav.dataset.active=link===links[links.length-1]?'end':'start';},{signal});}}
static init(){
...
slider.setAttribute('aria-valuetext',`$${slider.value}`);},{signal:this.#abortController.signal});}
static init(){
...
#radiusField;
#categorySelect;
#defaultCounts=new Map();#debounceTimer=null;
#fetchController=null;#currentPage=1;#isPaginating=false;#abortController=new AbortController();
constructor(page){
this.#page=page;
...
this.#abortController.abort();
BrowseFilter.#instance=null;}#bind(){
const{signal}=this.#abortController;
...
window.addEventListener('pageshow',this.#handlePageshow,{signal});}#syncZipRequired=()=>{
if(!this.#zipField||!this.#radiusField)return;
this.#zipField.required=this.#radiusField.value!==''&&!(/^\d{5}$/).test(this.#zipField.value);};#getBasePath(){
return this.#form.action?new URL(this.#form.action).pathname:'/tools';}#buildQueryString(){
...
return params.size>0;}#showSkeletons(){
if(!this.#grid)return;
...
if(this.#emptyState)this.#emptyState.hidden=true;}#updateResultCount(data){
...
p.append('Showing ',rangeStrong,' of ',totalStrong,` ${suffix} within `,radiusStrong,' miles of ',zipStrong);}else{
p.append('Showing ',rangeStrong,' of ',totalStrong,` ${suffix}`);}}else{
p.textContent='No tools match your filters.';}
if(existing){
existing.replaceWith(p);}else{
this.#countArea.appendChild(p);}}#updateClearButton(){
...
fieldset.appendChild(link);}else if(!active&&existing){
existing.remove();}}#updateCategoryCounts(counts){
...
opt.textContent=`${name} (${n})`;}else{
opt.textContent=this.#defaultCounts.get(opt.value)??opt.textContent;}}}#updateEmptyState(){
...
this.#emptyState.appendChild(clearLink);}else{
const p=document.createElement('p');
...
if(showLoading){
this.#showSkeletons();}else if(this.#grid){
...
currentPagination.outerHTML=data.paginationHtml;}else{
currentPagination.remove();}}else if(data.paginationHtml.trim()&&this.#grid){
...
this.#emptyState.hidden=false;}}else{
if(this.#grid)this.#grid.hidden=false;
if(this.#emptyState)this.#emptyState.hidden=true;}
if(replaceHistory){
history.replaceState(null,'',url);}else{
...
this.#form.submit();}}#handleSubmit=(e)=>{
e.preventDefault();
clearTimeout(this.#debounceTimer);
this.#currentPage=1;
this.#fetchFiltered({showLoading:false});};#handleImmediateChange=()=>{
clearTimeout(this.#debounceTimer);
this.#currentPage=1;
this.#fetchFiltered();};#handleDebouncedInput=()=>{
clearTimeout(this.#debounceTimer);
this.#currentPage=1;
this.#debounceTimer=setTimeout(()=>this.#fetchFiltered({showLoading:false}),BrowseFilter.#DEBOUNCE_MS);};#handlePageClick=(e)=>{
...
el.selectedIndex=0;}else if(el.type==='range'){
el.value=el.max;}else{
el.value='';}}
const defaultZip=this.#form.dataset.defaultZip||'';
const defaultRadius=this.#form.dataset.defaultRadius||'';
if(defaultZip&&defaultRadius){
...
this.#syncZipRequired();
this.#currentPage=1;
this.#fetchFiltered();}};#handlePopstate=()=>{
...
if(option)el.value=option.value;}else if(el.type!=='hidden'){
el.value=urlValue??el.defaultValue;}}
this.#syncZipRequired();
this.#currentPage=parseInt(params.get('page')??'1',10);
this.#fetchFiltered({replaceHistory:true});};#handleBeforeUnload=()=>{
clearTimeout(this.#debounceTimer);
this.#fetchController?.abort();};#handlePageshow=(e)=>{
...
#gridBtn;
#listBtn;
#mql;#abortController=new AbortController();
...
destroy(){
this.#abortController.abort();
ViewToggle.#instance=null;}#getPreferred(){
return localStorage.getItem(ViewToggle.#STORAGE_KEY)==='list'?'list':'grid';}#setView(mode){
...
if(!this.#mql.matches)localStorage.setItem(ViewToggle.#STORAGE_KEY,mode);}#handleClick=(e)=>{
...
#cancelBtn;
#labelEl;
#confirmIcon;#mode=null;#file=null;#objectUrl=null;#focalX=50;#focalY=50;#repositionImageId=null;#layout=null;#frameLeft=0;#frameTop=0;#savedScrollY=0;#onConfirmCallback=null;#dragging=false;#startX=0;#startY=0;#startFrameLeft=0;#startFrameTop=0;#abortController=new AbortController();
constructor(dialog){
this.#dialog=dialog;
...
return Math.max(min,Math.min(max,val));}#cleanupCropState(){
if(this.#objectUrl){
...
if(fi)fi.value='';}#closeCropDialog(){
this.#cleanupCropState();
this.#dialog.close();}#layoutCrop(){
...
this.#positionFrameFromFocal();}#positionFrameFromFocal(){
...
`left:${this.#frameLeft}px;top:${this.#frameTop}px`);}#setFramePosition(left,top){
this.#frameLeft=left;
this.#frameTop=top;
NT.style.setRule('crop-frame-pos','#crop-frame',
`left:${left}px;top:${top}px`);}#updateFocalFromFrame(){
if(!this.#layout)return;
const{imgLeft,imgTop,dispW,dispH,frameW,frameH}=this.#layout;
const maxOffsetX=dispW-frameW;
const maxOffsetY=dispH-frameH;
...
this.#focalY=maxOffsetY>0?ImageCrop.#clamp(Math.round((relTop/maxOffsetY)*100),0,100):50;}#openUpload(file,opts={}){
this.#mode='upload';
this.#file=file;
...
this.#viewport.focus();}#openReposition(imgSrc,focalX,focalY,imageId){
...
this.#savedScrollY=window.scrollY;
this.#dialog.showModal();
this.#viewport.focus();}#setConfirmState(disabled,loading){
if(this.#confirmBtn){
this.#confirmBtn.disabled=disabled;
if(loading)this.#confirmBtn.dataset.loading='';
else delete this.#confirmBtn.dataset.loading;}}#handlePointerDown=(e)=>{
if(!this.#layout)return;
e.preventDefault();
...
this.#startFrameTop=this.#frameTop;};#handlePointerMove=(e)=>{
...
this.#updateFocalFromFrame();};#handlePointerUp=()=>{this.#dragging=false;};
#handleKeydown=(e)=>{
const step=1;
let handled=false;
...
if(handled){
e.preventDefault();
this.#positionFrameFromFocal();}};#handleResize=()=>{
if(this.#dialog.open)this.#layoutCrop();};#handleConfirm=()=>{
if(this.#onConfirmCallback){
this.#onConfirmCallback(this.#mode,{
file:this.#file,
focalX:this.#focalX,
focalY:this.#focalY,
imageId:this.#repositionImageId,});}};#handleDialogClose=()=>{
this.#cleanupCropState();
...
#dropZone;
#fileInput;
#dataContainer;#queuedPhotos=[];#primaryIndex=0;#repositionIndex=-1;#dragItem=null;#dragCounter=0;#abortController=new AbortController();
...
requestAnimationFrame(()=>{
lastLi?.scrollIntoView({behavior:'smooth',block:'nearest'});});}else if(mode==='reposition'&&this.#repositionIndex>=0){
...
this.#dataContainer.appendChild(altInput);}}#updateHint(){
...
this.#dropZone.hidden=remaining<=0;}#renderQueue(){
...
primaryLabel.append(starIcon,' Primary');}else{
...
if(cropHint)cropHint.hidden=this.#queuedPhotos.length===0;}#handleQueueClick=(e)=>{
...
this.#primaryIndex=0;}else if(this.#primaryIndex>idx){
this.#primaryIndex--;}
...
const blobUrl=URL.createObjectURL(entry.file);
NT.crop.openReposition(blobUrl,entry.focalX,entry.focalY,`queue-${this.#repositionIndex}`);}};#handleQueueInput=(e)=>{
const altInput=e.target.closest('[data-alt-queue]');
if(!altInput)return;
const idx=parseInt(altInput.dataset.altQueue,10);
if(this.#queuedPhotos[idx]){
this.#queuedPhotos[idx].altText=altInput.value;}};#handleDragStart=(e)=>{
...
e.dataTransfer.setData('text/plain',li.dataset.queueIndex);};#handleDragEnd=(e)=>{
const li=e.target.closest('li[data-queue-index]');
if(li)delete li.dataset.dragging;
this.#dragItem=null;
for(const el of this.#queue.querySelectorAll('[data-drag-over]')){
delete el.dataset.dragOver;}};#handleQueueDragOver=(e)=>{
e.preventDefault();
...
target.dataset.dragOver='';};#handleQueueDrop=(e)=>{
e.preventDefault();
const target=e.target.closest('li[data-queue-index]');
...
this.#renderQueue();
this.#queue.querySelectorAll('li[data-queue-index]')[idx-1]?.focus();}else if((e.key==='ArrowDown'||e.key==='ArrowRight')&&idx<items.length-1){
e.preventDefault();
this.#primaryIndex=PhotoQueue.#reindexPrimary(this.#primaryIndex,idx,idx+1);
const[moved]=this.#queuedPhotos.splice(idx,1);
this.#queuedPhotos.splice(idx+1,0,moved);
this.#renderQueue();
this.#queue.querySelectorAll('li[data-queue-index]')[idx+1]?.focus();}};#handleDropZoneClick=(e)=>{
if(e.target===this.#fileInput)return;
this.#fileInput.click();};#handleDropZoneKeydown=(e)=>{
if(e.key==='Enter'||e.key===' '){
e.preventDefault();
this.#fileInput.click();}};#handleFileChange=()=>{
...
if(NT.crop){
NT.crop.openUpload(file,{icon:'fa-solid fa-plus',label:'Add'});}};#handleDragEnter=(e)=>{
e.preventDefault();
this.#dragCounter++;
this.#dropZone.dataset.dragover='';};#handleDragOver=(e)=>{
e.preventDefault();
e.dataTransfer.dropEffect='copy';};#handleDragLeave=()=>{
this.#dragCounter--;
if(this.#dragCounter<=0){
this.#dragCounter=0;
delete this.#dropZone.dataset.dragover;}};#handleDrop=(e)=>{
e.preventDefault();
this.#dragCounter=0;
...
starIcon.className='fa-solid fa-star';
starIcon.setAttribute('aria-hidden','true');
label.append(starIcon,' Primary');}else{
label.textContent='Primary';}}
#fieldset;#toolId;#busy=false;#dragItem=null;#dragCounter=0;#abortController=new AbortController();
...
if(this.#busy)return;
if(mode==='upload'){
await this.#handleUpload(data);}else if(mode==='reposition'){
...
destroy(){
this.#abortController.abort();
GalleryManager.#instance=null;}#setBusy(state){
this.#busy=state;
const g=document.getElementById('gallery-manager');
if(g)g.ariaBusy=state?'true':'false';}#getImageIds(){
const g=document.getElementById('gallery-manager');
if(!g)return[];
...
return div.innerHTML.replace(/"/g,'&quot;');}#ensureGallery(){
...
NT.toast('Photo uploaded.','success');}catch{
NT.toast('Failed to upload photo.','error');
NT.crop.setConfirmState(false,false);}finally{
this.#setBusy(false);}}
...
img.dataset.focalY=String(data.focalY);}else{
delete img.dataset.focalX;
delete img.dataset.focalY;}
NT.applyFocalPoints(li);}}
NT.crop.close();
NT.toast('Position saved.','success');}catch{
NT.toast('Failed to save position.','error');
NT.crop.setConfirmState(false,false);}finally{
this.#setBusy(false);}}
...
target.after(this.#dragItem);}else{
...
e.preventDefault();
swapTarget.before(li);}else if((e.key==='ArrowDown'||e.key==='ArrowRight')&&idx<items.length-1){
swapTarget=items[idx+1];
e.preventDefault();
swapTarget.after(li);}
...
await this.#handleAltTextBlur(input);},true);}
...
const hint=this.#fieldset.querySelector('#gallery-crop-hint');
if(hint){
hint.after(empty);}else{
this.#fieldset.querySelector('legend').after(empty);}}
this.#updateSlotHint();
NT.toast('Photo deleted.','success');}catch{
NT.toast('Failed to delete photo.','error');
deleteBtn.disabled=false;}finally{
this.#setBusy(false);}}
...
NT.toast('Primary photo updated.','success');}catch{
NT.toast('Failed to set primary photo.','error');}finally{
for(const r of g.querySelectorAll('[data-primary-radio]')){
r.disabled=false;}
this.#setBusy(false);}}
...
if(res.ok){
NT.toast('Alt text saved.','success');}else{
NT.toast('Failed to save alt text.','error');}}catch{
NT.toast('Failed to save alt text.','error');}}#bindDropZone(){
const dropZone=document.getElementById('photo-drop-zone');
...
NT.crop?.openUpload(file);},{signal});
dropZone.addEventListener('dragenter',(e)=>{
e.preventDefault();
this.#dragCounter++;
dropZone.dataset.dragover='';},{signal});
dropZone.addEventListener('dragover',(e)=>{
e.preventDefault();
e.dataTransfer.dropEffect='copy';},{signal});
dropZone.addEventListener('dragleave',()=>{
this.#dragCounter--;
if(this.#dragCounter<=0){
this.#dragCounter=0;
delete dropZone.dataset.dragover;}},{signal});
...
#mainSource;
#lightbox;
#lightboxImg;#allImages=[];#currentIndex=0;#lightboxIndex=0;#triggerElement=null;#abortController=new AbortController();
...
destroy(){
this.#abortController.abort();
GalleryViewer.#instance=null;}#setActiveThumb(index){
...
this.#mainSource.hidden=false;}else{
...
this.#mainImg.dataset.focalY=fy;}else{
...
caption.hidden=altText==='';}}#bindThumbs(){
const{signal}=this.#abortController;
this.#thumbList.addEventListener('click',(e)=>{
const btn=e.target.closest('button');
if(!btn)return;
const index=Array.from(this.#thumbButtons).indexOf(btn);
if(index>=0)this.#setActiveThumb(index);},{signal});
this.#galleryEl.addEventListener('keydown',(e)=>{
if(this.#lightbox?.open)return;
if(e.key==='ArrowRight'||e.key==='ArrowDown'){
e.preventDefault();
this.#setActiveThumb((this.#currentIndex+1)%this.#thumbButtons.length);}else if(e.key==='ArrowLeft'||e.key==='ArrowUp'){
e.preventDefault();
this.#setActiveThumb((this.#currentIndex-1+this.#thumbButtons.length)%this.#thumbButtons.length);}},{signal});}#buildImageList(){
if(this.#thumbList){
for(const btn of this.#thumbList.querySelectorAll('button')){
this.#allImages.push({
src:btn.dataset.full,
alt:btn.dataset.alt||'',});}}else if(this.#mainImg){
const link=this.#mainImg.closest('a[data-lightbox-trigger]');
this.#allImages.push({
src:link?.href||this.#mainImg.src,
alt:this.#mainImg.alt,});}}#showLightboxImage(index){
...
if(nextBtn)nextBtn.hidden=this.#allImages.length<=1;}#bindLightbox(){
const{signal}=this.#abortController;
const prevBtn=document.getElementById('lightbox-prev');
const nextBtn=document.getElementById('lightbox-next');
...
this.#lightbox.showModal();},{signal});
closeBtn?.addEventListener('click',()=>this.#closeLightbox(),{signal});
this.#lightbox.addEventListener('click',(e)=>{
if(e.target===this.#lightbox)this.#closeLightbox();},{signal});
prevBtn?.addEventListener('click',()=>{
this.#showLightboxImage((this.#lightboxIndex-1+this.#allImages.length)%this.#allImages.length);},{signal});
nextBtn?.addEventListener('click',()=>{
this.#showLightboxImage((this.#lightboxIndex+1)%this.#allImages.length);},{signal});
this.#lightbox.addEventListener('keydown',(e)=>{
if(e.key==='Escape'){
e.preventDefault();
this.#closeLightbox();}else if(e.key==='ArrowLeft'&&this.#allImages.length>1){
e.preventDefault();
this.#showLightboxImage((this.#lightboxIndex-1+this.#allImages.length)%this.#allImages.length);}else if(e.key==='ArrowRight'&&this.#allImages.length>1){
e.preventDefault();
this.#showLightboxImage((this.#lightboxIndex+1)%this.#allImages.length);}},{signal});}#closeLightbox(){
this.#lightbox.close();
